...
describe('Unique Collection Storage Contract Tests', () => {
    ...
    var AccessControlManager, CollectionStorage, Collection, UniqueCollectionStorage, Erc721Contract;
    var collection;
    ...
    beforeEach(async () => {
        ...
        // launch unique collection storage contract
    ...
    describe("Unique Asset Info", () => {
        it('Set and Burn info', async () => {            
            ...
            // record info
            expect(await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData, 0, creatorAddress.address))
                .to.emit(uniqueCollectionStorage, "TokenRoyaltiesUpdated")
                .withArgs(0, [creatorAddress.address, receiverAddress.address], [15000, 10000]);;

            // check whether the info has been recorded
            var tokenFees = await uniqueCollectionStorage.getMultipleRoyalties(0, 1000000);
            ...
            expect(await uniqueCollectionStorage.isCreator(0, creatorAddress.address)).to.equal(true);
            assetData = await uniqueCollectionStorage.getAssetData(0);
            expect(assetData[0]).to.equal(0);
            expect(assetData[1]).to.equal(collection.address);
            expect(await uniqueCollectionStorage.isLocked(0)).to.equal(true);
            expect(await uniqueCollectionStorage.tokenURI(0, 100)).to.equal("arweave.net/tx/unique-uri-0");

            // burn info
            await uniqueCollectionStorage.burnUniqueAssetInfo(0);

            // check whether the info has been deleted
            await expect(uniqueCollectionStorage.getMultipleRoyalties(0, 1000000)).to.be.reverted;
            expect(await uniqueCollectionStorage.isCreator(0, creatorAddress.address)).to.equal(false);
            assetData = await uniqueCollectionStorage.getAssetData(0);
            expect(assetData[0]).to.equal(ethers.constants.AddressZero);
            expect(assetData[1]).to.equal(ethers.constants.AddressZero);
            expect(await uniqueCollectionStorage.isLocked(0)).to.equal(false);
            await expect(uniqueCollectionStorage.tokenURI(0, 0)).to.be.reverted;
        });   
    });
    ...
    describe("Uri Tests", () => {
        it('Update unique asset uri', async () => {
            ...
            await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData, 0, playerAddress.address);

            await expect(uniqueCollectionStorage.connect(playerAddress).setUniqueUri(0,"arweave.net/tx/unique-uri-0v2"))
                .to.emit(uniqueCollectionStorage, "UniqueUriUpdated")
                .withArgs(0, 1);

            expect(await uniqueCollectionStorage.tokenURI(0, 0)).to.equal("arweave.net/tx/unique-uri-0");
            expect(await uniqueCollectionStorage.tokenURI(0, 1)).to.equal("arweave.net/tx/unique-uri-0v2");
            expect(await uniqueCollectionStorage.tokenURI(0, 100)).to.equal("arweave.net/tx/unique-uri-0v2");
        });
    });
    ...
    describe("Royalty Tests", () => {
        ...
        it('Calculate multiple royalty amounts', async () => {
            ...
            await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData, 0, creatorAddress.address);

            var tokenFees = await uniqueCollectionStorage.getMultipleRoyalties(0, 100000);
            expect(tokenFees[0][0]).to.equal(developerAddress.address);
            expect(tokenFees[0][1]).to.equal(creatorAddress.address);
            expect(tokenFees[0][2]).to.equal(receiverAddress.address);
        ...
        it('Query original item royalty', async () => {
            ...
            var uniqueAssetCreateData2 = [creatorAddress.address, collection.address, false, 1, "", [creatorAddress.address], [10000]];

            await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData, 0, creatorAddress.address);

            var tokenFees = await uniqueCollectionStorage.getRoyalty(0, 1000000);
            ...
            await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData2, 1, creatorAddress.address);

            tokenFees = await uniqueCollectionStorage.getRoyalty(1, 1000000);
        ...
        it('Update original item royalty', async () => {
            var receivers = [creatorAddress.address, receiverAddress.address];
            var rates = [10000, 5000];
            var uniqueAssetCreateData = [creatorAddress.address, collection.address, false, 0, "", receivers, rates];
            ...
            var tokenFees2 = await uniqueCollectionStorage.getRoyalty(0, 1000000);
        ...
        it('Original royalty update pushes total over limit', async () => {
            ...
            var tokenFees2 = await uniqueCollectionStorage.getMultipleRoyalties(0, 20000);
            ...
            var tokenFees3 = await uniqueCollectionStorage.getMultipleRoyalties(0, 1000000);
        ...
        it('Update royalty to equal 2e5 and over 2e5', async () => {
            ...
            var uniqueAssetCreateData = [creatorAddress.address, sampleContract.address, false, 0, "", receivers, rates];

            await uniqueCollectionStorage.setUniqueAssetInfo(uniqueAssetCreateData, 0, creatorAddress.address);

            var tokenFee = await uniqueCollectionStorage.getRoyalty(0, 1000000);
            ...
            var tokenFee2 = await uniqueCollectionStorage.getRoyalty(0, 1000000);
            expect(tokenFee2.receiver).to.equal(developerAddress.address);
            expect(tokenFee2.royaltyAmount).to.equal(200000);

            var tokenFees2 = await uniqueCollectionStorage.getMultipleRoyalties(0, 200000);
            ...
            var tokenFee3 = await uniqueCollectionStorage.getRoyalty(0, 1000000);
            expect(tokenFee3.receiver).to.equal(developerAddress.address);
            expect(tokenFee3.royaltyAmount).to.equal(300000);

            var tokenFees3 = await uniqueCollectionStorage.getMultipleRoyalties(0, 50000);
            expect(tokenFees3[0][0]).to.equal(developerAddress.address);
